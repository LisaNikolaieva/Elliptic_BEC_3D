Config;
cd(ProjectDir)
load('a=50\Psi\g=0\Psi_e=0.995.mat')
...
ss=surf(rz*r_mult_microm,rx*r_mult_microm,squeeze(Rho_mult_cm3*abs(Psi(Ny/2,:,:)).^2)); ss.EdgeColor='none'; daspect([1 1 1]); view(2)
cb=colorbar;
...
ss=surf(rz*r_mult_microm,ry*r_mult_microm,squeeze(Rho_mult_cm3*abs(Psi(:,Nx/2,:)).^2)); ss.EdgeColor='none'; daspect([1 1 1]); view(2);
cb=colorbar;
cb.TickLabelInterpreter='latex';
    cb.Location='eastoutside';
    cb.Label.String='$\rho$, $1/$cm$^3$';
